...
import { REGISTRAR_NAV_MENU, NavItem } from '../../../shared/constants/nav-menu.constant';
import { AuthService } from '../../../core/services/auth.service';
import { User } from '../../../core/models/user.model';

@Component({
  selector: 'app-registrar-base',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
...
export class RegistrarBaseComponent implements OnInit {
  private readonly router = inject(Router);
  private readonly route = inject(ActivatedRoute);
  private readonly authService = inject(AuthService);

  navItems: NavItem[] = REGISTRAR_NAV_MENU;
  currentRoute = '';
  loggedInUser: string = '';
  ...
  private filterNavItemsByRole(): void {
    ...
    this.navItems = REGISTRAR_NAV_MENU.filter(item => {
  ...
  isActive(route: string): boolean {
    const fullRoute = `/registrar/${route}`;
